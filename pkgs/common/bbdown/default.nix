...
buildDotnetModule rec {
  ...
  '';
  dotnetFlags =
    ["-p:PublishReadyToRun=false"] # for some reason this is set to win-x64 in the project files
    ++ lib.optionals stdenv.hostPlatform.isDarwin ["-p:RuntimeIdentifier=${dotnetCorePackages.systemToDotnetRid stdenvNoCC.hostPlatform.system}"]
    ++ lib.optionals stdenv.hostPlatform.isLinux ["-p:RuntimeIdentifier="];
