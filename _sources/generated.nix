# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  aerospace = {
    pname = "aerospace";
    version = "0.20.1-Beta";
    src = fetchurl {
      url = "https://github.com/nikitabobko/AeroSpace/releases/download/v0.20.1-Beta/AeroSpace-v0.20.1-Beta.zip";
      sha256 = "sha256-aKrxN+JfksjEn8auqluXU3zPksExI+Z4RoLlPqo3Y8Y=";
    };
  };
  alive-progress = {
    pname = "alive-progress";
    version = "3.3.0";
    src = fetchurl {
      url = "https://pypi.org/packages/source/a/alive-progress/alive-progress-3.3.0.tar.gz";
      sha256 = "sha256-RX3SQotI2s1JhUAipGRI0jakjxtyd4dAccOTlTB+gww=";
    };
  };
  antifennel = {
    pname = "antifennel";
    version = "cea2f62b71c53eeb2bfe056625a87f1a9ddfcddf";
    src = fetchgit {
      url = "https://git.sr.ht/~technomancy/antifennel";
      rev = "cea2f62b71c53eeb2bfe056625a87f1a9ddfcddf";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-LPQLxq7WQHi6CXgdYy6FKz04efFT5p6xPwDx5qhAw50=";
    };
    date = "2025-10-05";
  };
  bash-env-json = {
    pname = "bash-env-json";
    version = "0.7.0";
    src = fetchurl {
      url = "https://github.com/tesujimath/bash-env-json/raw/0.7.0/bash-env-json";
      sha256 = "sha256-6x8zErIYZuk3qWB807rNVbvH9ZLl8cGffJnKrt7DlSg=";
    };
  };
  bbdown = {
    pname = "bbdown";
    version = "d869225aed7ccdb43e48277c6bd67c690d76f29e";
    src = fetchFromGitHub {
      owner = "nilaoda";
      repo = "BBDown";
      rev = "d869225aed7ccdb43e48277c6bd67c690d76f29e";
      fetchSubmodules = false;
      sha256 = "sha256-C84eQ3Y3/YHup1dx4BtH9qstRs3dgXMCAmZXemvG1Xc=";
    };
    date = "2025-10-23";
  };
  borders = {
    pname = "borders";
    version = "v1.8.4";
    src = fetchFromGitHub {
      owner = "FelixKratz";
      repo = "JankyBorders";
      rev = "v1.8.4";
      fetchSubmodules = false;
      sha256 = "sha256-31Er+cUQNJbZnXKC6KvlrBhOvyPAM7nP3BaxunAtvWg=";
    };
  };
  clash2singbox = {
    pname = "clash2singbox";
    version = "v0.1.4";
    src = fetchFromGitHub {
      owner = "xmdhs";
      repo = "clash2singbox";
      rev = "v0.1.4";
      fetchSubmodules = false;
      sha256 = "sha256-qEv5lmuu4UWaxmCQ0sb21TMJVhYaW7VscD6O+dOWjgE=";
    };
  };
  darkreader = {
    pname = "darkreader";
    version = "4.9.118";
    src = fetchurl {
      url = "https://github.com/darkreader/darkreader/releases/download/v4.9.118/darkreader-firefox.xpi";
      sha256 = "sha256-AVmAWk/jCz32D+t8udvd9q8euhjw3+te6FcrzKucPFw=";
    };
  };
  deeplx = {
    pname = "deeplx";
    version = "v1.1.0";
    src = fetchFromGitHub {
      owner = "OwO-Network";
      repo = "DeepLX";
      rev = "v1.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-LJC9iSH9XrX87hqhZVLgmSml7wLj6G0H6RE6kxKVyFg=";
    };
  };
  dict2xml = {
    pname = "dict2xml";
    version = "release-1.7.7";
    src = fetchFromGitHub {
      owner = "delfick";
      repo = "python-dict2xml";
      rev = "release-1.7.7";
      fetchSubmodules = false;
      sha256 = "sha256-66ODdslXF6nWYqJku8cNG0RPK/YGEfbpHwVLLnSoDrk=";
    };
  };
  dutis = {
    pname = "dutis";
    version = "v2.0.0";
    src = fetchFromGitHub {
      owner = "tsonglew";
      repo = "dutis";
      rev = "v2.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-YmkmHsA5PvAOowl9y7W8DB4wwdupKCxnQA1cuWxsiE8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-YmkmHsA5PvAOowl9y7W8DB4wwdupKCxnQA1cuWxsiE8=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  "emacs29.no-frame-refocus-cocoa" = {
    pname = "emacs29.no-frame-refocus-cocoa";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-28/no-frame-refocus-cocoa.patch";
      sha256 = "sha256-+1d33IkKoHNJ8UOuZcK89D7a1v6/1WSwGiI1xaFfyr0=";
    };
    date = "2026-01-02";
  };
  "emacs29.role-patch" = {
    pname = "emacs29.role-patch";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-28/fix-window-role.patch";
      sha256 = "sha256-H4Qj6n5uZsmsbdjjexGZctqhJk3gAXKiSnmnEO/LgTA=";
    };
    date = "2026-01-02";
  };
  "emacs29.round-undecorated-frame" = {
    pname = "emacs29.round-undecorated-frame";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-29/round-undecorated-frame.patch";
      sha256 = "sha256-dFH4D1WYQOVOagUuVdEQB3irxV+Y8dDAOKJOJXc/KHQ=";
    };
    date = "2026-01-02";
  };
  "emacs29.system-appearance" = {
    pname = "emacs29.system-appearance";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-28/system-appearance.patch";
      sha256 = "sha256-1u4VmDmzi2r1Ode5vf8jEmPkUcH9Qu7A0SUxjJ24zZI=";
    };
    date = "2026-01-02";
  };
  "emacs30.role-patch" = {
    pname = "emacs30.role-patch";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-28/fix-window-role.patch";
      sha256 = "sha256-H4Qj6n5uZsmsbdjjexGZctqhJk3gAXKiSnmnEO/LgTA=";
    };
    date = "2026-01-02";
  };
  "emacs30.round-undecorated-frame" = {
    pname = "emacs30.round-undecorated-frame";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-30/round-undecorated-frame.patch";
      sha256 = "sha256-dFH4D1WYQOVOagUuVdEQB3irxV+Y8dDAOKJOJXc/KHQ=";
    };
    date = "2026-01-02";
  };
  "emacs30.system-appearance" = {
    pname = "emacs30.system-appearance";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-30/system-appearance.patch";
      sha256 = "sha256-nrPOgGQAJb/5brrrWJNDARY2jWNJ9OsMtO+LPVhHfbY=";
    };
    date = "2026-01-02";
  };
  "emacs31.round-undecorated-frame" = {
    pname = "emacs31.round-undecorated-frame";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-31/round-undecorated-frame.patch";
      sha256 = "sha256-JpR7ZyT8KfrdRIiYCMXPC0zmJ4zwT0YIaiHfUMjEFR0=";
    };
    date = "2026-01-02";
  };
  "emacs31.system-appearance" = {
    pname = "emacs31.system-appearance";
    version = "e115ba40135305b3a58c69a64d03392b1d49e9ee";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/e115ba40135305b3a58c69a64d03392b1d49e9ee/patches/emacs-31/system-appearance.patch";
      sha256 = "sha256-Uyg1A9te0oh+nXM7qq+A8sgQ5mjngumIvaWFWgsevrQ=";
    };
    date = "2026-01-02";
  };
  emacsclient = {
    pname = "emacsclient";
    version = "917f2f2694019d534098f5e2e365b5f6e5ddbd37";
    src = fetchurl {
      url = "https://github.com/nashamri/spacemacs-logo/raw/917f2f2694019d534098f5e2e365b5f6e5ddbd37/spacemacs.icns";
      sha256 = "sha256-s9uLfPpLxbziS8TcHt47dSxxhse1TAmZTqtexOqkiQA=";
    };
    date = "2018-08-26";
  };
  fav = {
    pname = "fav";
    version = "v1.2.2";
    src = fetchFromGitHub {
      owner = "kingwingfly";
      repo = "fav";
      rev = "v1.2.2";
      fetchSubmodules = false;
      sha256 = "sha256-fN6X4q2mWKNUr6nEevCaY/Nd9Y8liJhffe3jhr6u+vg=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-fN6X4q2mWKNUr6nEevCaY_Nd9Y8liJhffe3jhr6u+vg=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  fisher = {
    pname = "fisher";
    version = "4.4.5";
    src = fetchFromGitHub {
      owner = "jorgebucaran";
      repo = "fisher";
      rev = "4.4.5";
      fetchSubmodules = false;
      sha256 = "sha256-VC8LMjwIvF6oG8ZVtFQvo2mGdyAzQyluAGBoK8N2/QM=";
    };
  };
  flexfox = {
    pname = "flexfox";
    version = "v5.7.5";
    src = fetchFromGitHub {
      owner = "yuuqilin";
      repo = "FlexFox";
      rev = "v5.7.5";
      fetchSubmodules = false;
      sha256 = "sha256-kw0PrQLXWDnwHZpQMMFL1J0SoQYpNOumvyW8WRftCYU=";
    };
  };
  go-musicfox = {
    pname = "go-musicfox";
    version = "v4.7.2";
    src = fetchFromGitHub {
      owner = "go-musicfox";
      repo = "go-musicfox";
      rev = "v4.7.2";
      fetchSubmodules = false;
      sha256 = "sha256-RJPb+aZawU22HBXTfr7+TP0ocFsNrP1mOHvHPRm2RnA=";
    };
  };
  httpx = {
    pname = "httpx";
    version = "0.28.1";
    src = fetchFromGitHub {
      owner = "encode";
      repo = "httpx";
      rev = "0.28.1";
      fetchSubmodules = false;
      sha256 = "sha256-tB8uZm0kPRnmeOvsDdrkrHcMVIYfGanB4l/xHsTKpgE=";
    };
  };
  jeejah = {
    pname = "jeejah";
    version = "c9c45fb4844035ffa71f33d8a571a532a6a82fd7";
    src = fetchgit {
      url = "https://gitlab.com/technomancy/jeejah";
      rev = "c9c45fb4844035ffa71f33d8a571a532a6a82fd7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-qNmLqHfHY1ksMpGFmIHQpzS3QKG81CGSnM79GaqZRFE=";
    };
    date = "2025-11-16";
  };
  jeejah-rock = {
    pname = "jeejah-rock";
    version = "0.3.2-1";
    src = fetchurl {
      url = "https://luarocks.org/manifests/technomancy/jeejah-0.3.2-1.rockspec";
      sha256 = "sha256-Ouj+umHFOLtGwwMKochvG1fqIyRMdPzXg9039jD/bPk=";
    };
  };
  lporg = {
    pname = "lporg";
    version = "v20.4.32";
    src = fetchFromGitHub {
      owner = "blacktop";
      repo = "lporg";
      rev = "v20.4.32";
      fetchSubmodules = false;
      sha256 = "sha256-g8zZZ/uQeoJQIzpQY3oMUuWMu4/8OnaOLeLvaii+Tmk=";
    };
  };
  mac-app-util = {
    pname = "mac-app-util";
    version = "4747968574ea58512c5385466400b2364c85d2d0";
    src = fetchurl {
      url = "https://github.com/hraban/mac-app-util/raw/4747968574ea58512c5385466400b2364c85d2d0/main.lisp";
      sha256 = "sha256-mDFxuZN2y3+FcMWh3YdmC0ZjjRfZxD7K6QbqX8BzkWI=";
    };
    date = "2025-12-26";
  };
  manix = {
    pname = "manix";
    version = "5a69f920f1501f3f7e71a275ab8dbfd4a9f85fb3";
    src = fetchFromGitHub {
      owner = "kaii-zen";
      repo = "manix";
      rev = "5a69f920f1501f3f7e71a275ab8dbfd4a9f85fb3";
      fetchSubmodules = false;
      sha256 = "sha256-Qxs6p+MyS/0NTbJzLKvZHgKBUE1+z+zaINouG//Uio8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Qxs6p+MyS_0NTbJzLKvZHgKBUE1+z+zaINouG__Uio8=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2021-07-28";
  };
  mkalias = {
    pname = "mkalias";
    version = "v0.3.2";
    src = fetchFromGitHub {
      owner = "reckenrode";
      repo = "mkAlias";
      rev = "v0.3.2";
      fetchSubmodules = false;
      sha256 = "sha256-tL3C/b2BPOGQpV287wECDCDWmKwwPvezAAN3qz7N07M=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-tL3C_b2BPOGQpV287wECDCDWmKwwPvezAAN3qz7N07M=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  musicdl = {
    pname = "musicdl";
    version = "1bac003d5513b1ba950c54508c2a3a6f6a6dead7";
    src = fetchFromGitHub {
      owner = "CharlesPikachu";
      repo = "musicdl";
      rev = "1bac003d5513b1ba950c54508c2a3a6f6a6dead7";
      fetchSubmodules = false;
      sha256 = "sha256-fRZpiB3ES67mp/nprujnffNr9n8simgNJmrWtM5XgX4=";
    };
    date = "2026-01-01";
  };
  musicn = {
    pname = "musicn";
    version = "v1.5.3-beta.0";
    src = fetchFromGitHub {
      owner = "zonemeen";
      repo = "musicn";
      rev = "v1.5.3-beta.0";
      fetchSubmodules = false;
      sha256 = "sha256-loIT6Oy0TmkqtjEUMOVZEk27hr5wWPiP5WycGDXmo3s=";
    };
  };
  nowplaying-cli = {
    pname = "nowplaying-cli";
    version = "v1.2.1";
    src = fetchFromGitHub {
      owner = "kirtan-shah";
      repo = "nowplaying-cli";
      rev = "v1.2.1";
      fetchSubmodules = false;
      sha256 = "sha256-FkyrtgsGzpK2rLNr+oxfPUbX43TVXYeiBg7CN1JUg8Y=";
    };
  };
  nu-lint = {
    pname = "nu-lint";
    version = "86d0583d1aa7d545434de963c846acf6c8ef4b2e";
    src = fetchFromGitHub {
      owner = "wvhulle";
      repo = "nu-lint";
      rev = "86d0583d1aa7d545434de963c846acf6c8ef4b2e";
      fetchSubmodules = false;
      sha256 = "sha256-Iraq0L+q1NPaYBn7+gYq5c6wKJmzKZutOO0ppvm3DcQ=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Iraq0L+q1NPaYBn7+gYq5c6wKJmzKZutOO0ppvm3DcQ=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-01-02";
  };
  pngpaste = {
    pname = "pngpaste";
    version = "0.2.3";
    src = fetchFromGitHub {
      owner = "jcsalterego";
      repo = "pngpaste";
      rev = "0.2.3";
      fetchSubmodules = false;
      sha256 = "sha256-uvajxSelk1Wfd5is5kmT2fzDShlufBgC0PDCeabEOSE=";
    };
  };
  pragmasevka = {
    pname = "pragmasevka";
    version = "33.3.6";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v33.3.6/pragma-sevka-mono.zip";
      sha256 = "sha256-YIuMi08Ybl1/WcCOeIy0wxX2LpmHbHAy0A3qNiZvmmI=";
    };
  };
  pragmasevka-nerd = {
    pname = "pragmasevka-nerd";
    version = "33.3.6";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v33.3.6/pragma-sevka-mono-nerd.zip";
      sha256 = "sha256-2a/yUc0+qF5yVVIdx+EJvgygaWqLUN8cwD9oB3a56tE=";
    };
  };
  pragmasevka-sans = {
    pname = "pragmasevka-sans";
    version = "33.3.6";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v33.3.6/pragma-sevka-sans.zip";
      sha256 = "sha256-LUZp2Gcc3igwMJrX+O+/UBAjS136J/EFeascWBdu0ac=";
    };
  };
  pragmasevka-sc = {
    pname = "pragmasevka-sc";
    version = "33.3.6";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v33.3.6/Pragma-Sevka-SC.ttf";
      sha256 = "sha256-n+deQ38Z6fmNi1lJFt38QjAO5dlnKvdIPb5eHMS5rMA=";
    };
  };
  pragmasevka-sc-nf = {
    pname = "pragmasevka-sc-nf";
    version = "33.3.6";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v33.3.6/Pragma-Sevka-SC-NF.ttf";
      sha256 = "sha256-1LjRuUDa7/LCEBkmRYBDkLnlgtukTv5NRnnmk+eFkY0=";
    };
  };
  pragmasevka-serif = {
    pname = "pragmasevka-serif";
    version = "33.3.6";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v33.3.6/pragma-sevka-serif.zip";
      sha256 = "sha256-R10JS0Se9kdGd/KrIVfNgFmrreaxbwQHq/jF30o2u+4=";
    };
  };
  pyfreeproxy = {
    pname = "pyfreeproxy";
    version = "b914bfbab2606012c963f2329df5e5ed0816bf85";
    src = fetchFromGitHub {
      owner = "CharlesPikachu";
      repo = "freeproxy";
      rev = "b914bfbab2606012c963f2329df5e5ed0816bf85";
      fetchSubmodules = false;
      sha256 = "sha256-8p5t+Dv10NWx8AGmvTgVoY05e/fEoo9iv/3ktJd1YVk=";
    };
    date = "2026-01-02";
  };
  rime-ice = {
    pname = "rime-ice";
    version = "3a9a76e2c58181d3980a1cdeb6dda55a67605f2d";
    src = fetchFromGitHub {
      owner = "iDvel";
      repo = "rime-ice";
      rev = "3a9a76e2c58181d3980a1cdeb6dda55a67605f2d";
      fetchSubmodules = false;
      sha256 = "sha256-FWPh3RJMNhbhUysE8B2C0b+Wz2Bo/oSA49h1HzRBCAI=";
    };
    date = "2025-12-31";
  };
  sarasa-term = {
    pname = "sarasa-term";
    version = "2.3.1";
    src = fetchurl {
      url = "https://github.com/laishulu/Sarasa-Term-SC-Nerd/releases/download/v2.3.1/SarasaTermSCNerd.ttc.tar.gz";
      sha256 = "sha256-WiXDbXdpRmf4R8IkdeIC3jUuwRKPUFUIHT6CzYoDmuI=";
    };
  };
  sd = {
    pname = "sd";
    version = "c730f8ad66a433ce7c9da954a77e81cca79bdce0";
    src = fetchFromGitHub {
      owner = "shanyouli";
      repo = "sd";
      rev = "c730f8ad66a433ce7c9da954a77e81cca79bdce0";
      fetchSubmodules = false;
      sha256 = "sha256-azFuxM/BFrDyTb3BlL119N77RRLTIXhcftl91iHcoyE=";
    };
    date = "2025-09-13";
  };
  seam = {
    pname = "seam";
    version = "v_cli.0.1.39";
    src = fetchFromGitHub {
      owner = "Borber";
      repo = "seam";
      rev = "v_cli.0.1.39";
      fetchSubmodules = false;
      sha256 = "sha256-rWbswgVQNA006gi9G+Vma+y0NbMDC0/P6SqHwxxvky8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-rWbswgVQNA006gi9G+Vma+y0NbMDC0_P6SqHwxxvky8=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  sidebery = {
    pname = "sidebery";
    version = "v5.4.0";
    src = fetchFromGitHub {
      owner = "mbnuqw";
      repo = "sidebery";
      rev = "v5.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-Y7Aq+fZPJcYQcJjykyobT5LFanz4TcMSNoJBWgr1w9Q=";
    };
  };
  singboxp = {
    pname = "singboxp";
    version = "c590569f066ac178c5f5d650a51707dc2ffaf0aa";
    src = fetchFromGitHub {
      owner = "PuerNya";
      repo = "sing-box";
      rev = "c590569f066ac178c5f5d650a51707dc2ffaf0aa";
      fetchSubmodules = false;
      sha256 = "sha256-/lqfUG23BMFV42ynHdmBk4qWauGMikxrp0DwI+x96eY=";
    };
    date = "2026-01-01";
  };
  switchaudio-osx = {
    pname = "switchaudio-osx";
    version = "1.2.2";
    src = fetchFromGitHub {
      owner = "deweller";
      repo = "switchaudio-osx";
      rev = "1.2.2";
      fetchSubmodules = false;
      sha256 = "sha256-AZJn5kHK/al94ONfIHcG+W0jyMfgdJkIngN+PVj+I44=";
    };
  };
  tmux-fastcopy = {
    pname = "tmux-fastcopy";
    version = "v0.14.1";
    src = fetchFromGitHub {
      owner = "abhinav";
      repo = "tmux-fastcopy";
      rev = "v0.14.1";
      fetchSubmodules = false;
      sha256 = "sha256-j7Kk5uaC3vx4JifaAHXfLvXx6ask9YqlNKEiU5BYi+A=";
    };
  };
  userChromeJS = {
    pname = "userChromeJS";
    version = "3868c57a0941e856538380e6a612e41d5ed605e3";
    src = fetchFromGitHub {
      owner = "benzBrake";
      repo = "userChrome.js-Loader";
      rev = "3868c57a0941e856538380e6a612e41d5ed605e3";
      fetchSubmodules = false;
      sha256 = "sha256-d/T6qU1X8cpwSTdA81OI4ZQRM4NRszIRTFNnuYkOXHg=";
    };
    date = "2025-12-19";
  };
  websocket-bridge-python = {
    pname = "websocket-bridge-python";
    version = "0.0.2";
    src = fetchurl {
      url = "https://pypi.org/packages/source/w/websocket_bridge_python/websocket_bridge_python-0.0.2.tar.gz";
      sha256 = "sha256-J9HGD3PSavZQwh2eZPbM6isdknZ1M2nkL5WHlYjRju8=";
    };
  };
  yabai-zsh-completions = {
    pname = "yabai-zsh-completions";
    version = "6e38681a002e13bdcd43f461f73c53b7c11fd4e5";
    src = fetchFromGitHub {
      owner = "Amar1729";
      repo = "yabai-zsh-completions";
      rev = "6e38681a002e13bdcd43f461f73c53b7c11fd4e5";
      fetchSubmodules = false;
      sha256 = "sha256-II00E32Pnt7PO+PcTtWp4NzSUDhQJTgAPw9HdlItbhQ=";
    };
    date = "2023-11-13";
  };
  zpmod = {
    pname = "zpmod";
    version = "v1.1.0";
    src = fetchFromGitHub {
      owner = "z-shell";
      repo = "zpmod";
      rev = "v1.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-vCQOn+8l9DcVJ6XbtHhr7N5IDr2Ht/JRub217ZO96Ok=";
    };
  };
}
