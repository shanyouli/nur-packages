# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  alive-progress = {
    pname = "alive-progress";
    version = "3.3.0";
    src = fetchurl {
      url = "https://pypi.org/packages/source/a/alive-progress/alive-progress-3.3.0.tar.gz";
      sha256 = "sha256-RX3SQotI2s1JhUAipGRI0jakjxtyd4dAccOTlTB+gww=";
    };
  };
  antifennel = {
    pname = "antifennel";
    version = "cea2f62b71c53eeb2bfe056625a87f1a9ddfcddf";
    src = fetchgit {
      url = "https://git.sr.ht/~technomancy/antifennel";
      rev = "cea2f62b71c53eeb2bfe056625a87f1a9ddfcddf";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-LPQLxq7WQHi6CXgdYy6FKz04efFT5p6xPwDx5qhAw50=";
    };
    date = "20251005";
  };
  bash-env-json = {
    pname = "bash-env-json";
    version = "0.7.0";
    src = fetchurl {
      url = "https://github.com/tesujimath/bash-env-json/raw/0.7.0/bash-env-json";
      sha256 = "sha256-6x8zErIYZuk3qWB807rNVbvH9ZLl8cGffJnKrt7DlSg=";
    };
  };
  bbdown = {
    pname = "bbdown";
    version = "259a5558cee0a349a7ebb60bd31e40c88e5bc1ed";
    src = fetchFromGitHub {
      owner = "nilaoda";
      repo = "BBDown";
      rev = "259a5558cee0a349a7ebb60bd31e40c88e5bc1ed";
      fetchSubmodules = false;
      sha256 = "sha256-5c+tvWOx9BMOfqZGGkPw86abhfezYXVpz1J/RLEQnA8=";
    };
    date = "20260110";
  };
  borders = {
    pname = "borders";
    version = "v1.8.4";
    src = fetchFromGitHub {
      owner = "FelixKratz";
      repo = "JankyBorders";
      rev = "v1.8.4";
      fetchSubmodules = false;
      sha256 = "sha256-31Er+cUQNJbZnXKC6KvlrBhOvyPAM7nP3BaxunAtvWg=";
    };
  };
  clash2singbox = {
    pname = "clash2singbox";
    version = "v0.1.4";
    src = fetchFromGitHub {
      owner = "xmdhs";
      repo = "clash2singbox";
      rev = "v0.1.4";
      fetchSubmodules = false;
      sha256 = "sha256-qEv5lmuu4UWaxmCQ0sb21TMJVhYaW7VscD6O+dOWjgE=";
    };
  };
  darkreader = {
    pname = "darkreader";
    version = "4.9.124";
    src = fetchurl {
      url = "https://github.com/darkreader/darkreader/releases/download/v4.9.124/darkreader-firefox.xpi";
      sha256 = "sha256-0RS8qFtPsuOHCnx4Z67wtQc4rNNltYipRMrtrwCpKQ4=";
    };
  };
  deeplx = {
    pname = "deeplx";
    version = "v1.1.1";
    src = fetchFromGitHub {
      owner = "OwO-Network";
      repo = "DeepLX";
      rev = "v1.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-M51z7iXJmYSRc9bsVPsbhD3v8Fq8Gwg+CO37B18wRZo=";
    };
  };
  dict2xml = {
    pname = "dict2xml";
    version = "release-1.7.8";
    src = fetchFromGitHub {
      owner = "delfick";
      repo = "python-dict2xml";
      rev = "release-1.7.8";
      fetchSubmodules = false;
      sha256 = "sha256-wCspFcqn6uAvecxx4Agzg7N3ps82mg8ukmmGwhfgajk=";
    };
  };
  dutis = {
    pname = "dutis";
    version = "v2.3.1";
    src = fetchFromGitHub {
      owner = "tsonglew";
      repo = "dutis";
      rev = "v2.3.1";
      fetchSubmodules = false;
      sha256 = "sha256-90BVsQdFCC+uqvd1EvY/t+242AmQuTalD5dSiA4n2aA=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-90BVsQdFCC+uqvd1EvY_t+242AmQuTalD5dSiA4n2aA=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  emacs-reader = {
    pname = "emacs-reader";
    version = "9824fc91eb51bec0edb8c3634a74d73226d26525";
    src = fetchgit {
      url = "https://codeberg.org/divyaranjan/emacs-reader";
      rev = "9824fc91eb51bec0edb8c3634a74d73226d26525";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-84v8NzAjH0djD98RKElzy3dIkSSh1c3OyjrHXR8cQrY=";
    };
    date = "20260415";
  };
  "emacs29.no-frame-refocus-cocoa" = {
    pname = "emacs29.no-frame-refocus-cocoa";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-28/no-frame-refocus-cocoa.patch";
      sha256 = "sha256-+1d33IkKoHNJ8UOuZcK89D7a1v6/1WSwGiI1xaFfyr0=";
    };
    date = "2026-04-24";
  };
  "emacs29.role-patch" = {
    pname = "emacs29.role-patch";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-28/fix-window-role.patch";
      sha256 = "sha256-H4Qj6n5uZsmsbdjjexGZctqhJk3gAXKiSnmnEO/LgTA=";
    };
    date = "2026-04-24";
  };
  "emacs29.round-undecorated-frame" = {
    pname = "emacs29.round-undecorated-frame";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-29/round-undecorated-frame.patch";
      sha256 = "sha256-dFH4D1WYQOVOagUuVdEQB3irxV+Y8dDAOKJOJXc/KHQ=";
    };
    date = "2026-04-24";
  };
  "emacs29.system-appearance" = {
    pname = "emacs29.system-appearance";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-28/system-appearance.patch";
      sha256 = "sha256-1u4VmDmzi2r1Ode5vf8jEmPkUcH9Qu7A0SUxjJ24zZI=";
    };
    date = "2026-04-24";
  };
  "emacs30.ns-mac-input-source" = {
    pname = "emacs30.ns-mac-input-source";
    version = "f26695ea0e759c707f610e22f55230c4e9d85d00";
    src = fetchurl {
      url = "https://github.com/LuciusChen/.emacs.d/raw/f26695ea0e759c707f610e22f55230c4e9d85d00/scripts/macos/patches/emacs-30/ns-alpha-background.patch";
      sha256 = "sha256-izzfjAWg4e8yYDuODXKIgwG7TdQUg2Su6ZebyzCg9hM=";
    };
    date = "2026-04-17";
  };
  "emacs30.role-patch" = {
    pname = "emacs30.role-patch";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-28/fix-window-role.patch";
      sha256 = "sha256-H4Qj6n5uZsmsbdjjexGZctqhJk3gAXKiSnmnEO/LgTA=";
    };
    date = "2026-04-24";
  };
  "emacs30.round-undecorated-frame" = {
    pname = "emacs30.round-undecorated-frame";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-30/round-undecorated-frame.patch";
      sha256 = "sha256-dFH4D1WYQOVOagUuVdEQB3irxV+Y8dDAOKJOJXc/KHQ=";
    };
    date = "2026-04-24";
  };
  "emacs30.system-appearance" = {
    pname = "emacs30.system-appearance";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-30/system-appearance.patch";
      sha256 = "sha256-nrPOgGQAJb/5brrrWJNDARY2jWNJ9OsMtO+LPVhHfbY=";
    };
    date = "2026-04-24";
  };
  "emacs31.ns-mac-input-source" = {
    pname = "emacs31.ns-mac-input-source";
    version = "f26695ea0e759c707f610e22f55230c4e9d85d00";
    src = fetchurl {
      url = "https://github.com/LuciusChen/.emacs.d/raw/f26695ea0e759c707f610e22f55230c4e9d85d00/scripts/macos/patches/emacs-31/ns-mac-input-source.patch";
      sha256 = "sha256-mf77qesHJXQ6kBzZXg3rV1GT4dJYCUy1bMusN62g/6I=";
    };
    date = "2026-04-17";
  };
  "emacs31.round-undecorated-frame" = {
    pname = "emacs31.round-undecorated-frame";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-31/round-undecorated-frame.patch";
      sha256 = "sha256-JpR7ZyT8KfrdRIiYCMXPC0zmJ4zwT0YIaiHfUMjEFR0=";
    };
    date = "2026-04-24";
  };
  "emacs31.system-appearance" = {
    pname = "emacs31.system-appearance";
    version = "55b91779432576a98d4cb7a0215237fb21a1cb0d";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/55b91779432576a98d4cb7a0215237fb21a1cb0d/patches/emacs-31/system-appearance.patch";
      sha256 = "sha256-Uyg1A9te0oh+nXM7qq+A8sgQ5mjngumIvaWFWgsevrQ=";
    };
    date = "2026-04-24";
  };
  emacsclient = {
    pname = "emacsclient";
    version = "917f2f2694019d534098f5e2e365b5f6e5ddbd37";
    src = fetchurl {
      url = "https://github.com/nashamri/spacemacs-logo/raw/917f2f2694019d534098f5e2e365b5f6e5ddbd37/spacemacs.icns";
      sha256 = "sha256-s9uLfPpLxbziS8TcHt47dSxxhse1TAmZTqtexOqkiQA=";
    };
    date = "2018-08-26";
  };
  emt = {
    pname = "emt";
    version = "b7f46fae280dba3269e9110f3cb61d2dd4fa67cc";
    src = fetchFromGitHub {
      owner = "roife";
      repo = "emt";
      rev = "b7f46fae280dba3269e9110f3cb61d2dd4fa67cc";
      fetchSubmodules = false;
      sha256 = "sha256-ewHZxytYw2NPkhF4zdl/fRSiBSN2jZAkdM4Ykb1x90Q=";
    };
    date = "20250102";
  };
  ewt-rs = {
    pname = "ewt-rs";
    version = "v0.5.0";
    src = fetchFromGitHub {
      owner = "Master-Hash";
      repo = "ewt-rs";
      rev = "v0.5.0";
      fetchSubmodules = false;
      sha256 = "sha256-i8886lu0KZpjfD35X0A7zitUSld+mCbz5uI7+nFvnZ4=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-i8886lu0KZpjfD35X0A7zitUSld+mCbz5uI7+nFvnZ4=/Cargo.lock";
      outputHashes = {
        "rust_icu_common-5.6.0" = "sha256-p0Xkz9kLPO46pd0W2fEQazRYku5AR3eCqaAQ4wN1gE0=";
      };
    };
  };
  fav = {
    pname = "fav";
    version = "v1.2.4";
    src = fetchFromGitHub {
      owner = "kingwingfly";
      repo = "fav";
      rev = "v1.2.4";
      fetchSubmodules = false;
      sha256 = "sha256-BGYSPIDEOlG+CYxfCVikvWLeyMT6rRTnN2xrGXdBsds=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-BGYSPIDEOlG+CYxfCVikvWLeyMT6rRTnN2xrGXdBsds=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  fisher = {
    pname = "fisher";
    version = "4.4.8";
    src = fetchFromGitHub {
      owner = "jorgebucaran";
      repo = "fisher";
      rev = "4.4.8";
      fetchSubmodules = false;
      sha256 = "sha256-Sf671UGOQXtOMrqoEOIBG5TCt0p5fd+aKGF2ExImbbs=";
    };
  };
  flexfox = {
    pname = "flexfox";
    version = "v6.0.6";
    src = fetchFromGitHub {
      owner = "yuuqilin";
      repo = "FlexFox";
      rev = "v6.0.6";
      fetchSubmodules = false;
      sha256 = "sha256-DmrvmQmhYvmAMRGg9aA+lyuwnO3HY8GkM4RBEnWP9ZU=";
    };
  };
  go-musicfox = {
    pname = "go-musicfox";
    version = "v4.8.5";
    src = fetchFromGitHub {
      owner = "go-musicfox";
      repo = "go-musicfox";
      rev = "v4.8.5";
      fetchSubmodules = false;
      sha256 = "sha256-fgzj1L/oIVMCfiWaB83e9GEMyn6lfFSzF/hqq+i+5I0=";
    };
  };
  httpx = {
    pname = "httpx";
    version = "0.28.1";
    src = fetchFromGitHub {
      owner = "encode";
      repo = "httpx";
      rev = "0.28.1";
      fetchSubmodules = false;
      sha256 = "sha256-tB8uZm0kPRnmeOvsDdrkrHcMVIYfGanB4l/xHsTKpgE=";
    };
  };
  jeejah = {
    pname = "jeejah";
    version = "c9c45fb4844035ffa71f33d8a571a532a6a82fd7";
    src = fetchgit {
      url = "https://gitlab.com/technomancy/jeejah";
      rev = "c9c45fb4844035ffa71f33d8a571a532a6a82fd7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-qNmLqHfHY1ksMpGFmIHQpzS3QKG81CGSnM79GaqZRFE=";
    };
    date = "20251116";
  };
  jeejah-rock = {
    pname = "jeejah-rock";
    version = "0.3.2-1";
    src = fetchurl {
      url = "https://luarocks.org/manifests/technomancy/jeejah-0.3.2-1.rockspec";
      sha256 = "sha256-Ouj+umHFOLtGwwMKochvG1fqIyRMdPzXg9039jD/bPk=";
    };
  };
  lporg = {
    pname = "lporg";
    version = "v20.4.32";
    src = fetchFromGitHub {
      owner = "blacktop";
      repo = "lporg";
      rev = "v20.4.32";
      fetchSubmodules = false;
      sha256 = "sha256-g8zZZ/uQeoJQIzpQY3oMUuWMu4/8OnaOLeLvaii+Tmk=";
    };
  };
  mac-app-util = {
    pname = "mac-app-util";
    version = "4747968574ea58512c5385466400b2364c85d2d0";
    src = fetchurl {
      url = "https://github.com/hraban/mac-app-util/raw/4747968574ea58512c5385466400b2364c85d2d0/main.lisp";
      sha256 = "sha256-mDFxuZN2y3+FcMWh3YdmC0ZjjRfZxD7K6QbqX8BzkWI=";
    };
    date = "2025-12-26";
  };
  manix = {
    pname = "manix";
    version = "5a69f920f1501f3f7e71a275ab8dbfd4a9f85fb3";
    src = fetchFromGitHub {
      owner = "kaii-zen";
      repo = "manix";
      rev = "5a69f920f1501f3f7e71a275ab8dbfd4a9f85fb3";
      fetchSubmodules = false;
      sha256 = "sha256-Qxs6p+MyS/0NTbJzLKvZHgKBUE1+z+zaINouG//Uio8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Qxs6p+MyS_0NTbJzLKvZHgKBUE1+z+zaINouG__Uio8=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "20210728";
  };
  mkalias = {
    pname = "mkalias";
    version = "v0.4.0";
    src = fetchFromGitHub {
      owner = "reckenrode";
      repo = "mkAlias";
      rev = "v0.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-YUvxBEwXu1LSYC+7g6inBizqgdjKnLugUTXtpSfayxI=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-YUvxBEwXu1LSYC+7g6inBizqgdjKnLugUTXtpSfayxI=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  "mtranserver.linux.amd64" = {
    pname = "mtranserver.linux.amd64";
    version = "4.0.33";
    src = fetchurl {
      url = "https://github.com/xxnuo/MTranServer/releases/download/v4.0.33/mtranserver-4.0.33-linux-amd64";
      sha256 = "sha256-nVbDsdMckZOAUO51D39LqO96oXiHLXGcGYm+SAXPu5U=";
    };
  };
  "mtranserver.linux.arm64" = {
    pname = "mtranserver.linux.arm64";
    version = "4.0.33";
    src = fetchurl {
      url = "https://github.com/xxnuo/MTranServer/releases/download/v4.0.33/mtranserver-4.0.33-linux-arm64";
      sha256 = "sha256-Yrbkhjj1Akvp4MPSB5gU7wU2LBVms47sOnBrggY8Pys=";
    };
  };
  "mtranserver.mac.amd64" = {
    pname = "mtranserver.mac.amd64";
    version = "4.0.33";
    src = fetchurl {
      url = "https://github.com/xxnuo/MTranServer/releases/download/v4.0.33/mtranserver-4.0.33-darwin-amd64";
      sha256 = "sha256-jfwUg3V1I3fpzMZFn6DYGducPoc6ote0eKUk7f9LIDw=";
    };
  };
  "mtranserver.mac.arm64" = {
    pname = "mtranserver.mac.arm64";
    version = "4.0.33";
    src = fetchurl {
      url = "https://github.com/xxnuo/MTranServer/releases/download/v4.0.33/mtranserver-4.0.33-darwin-arm64";
      sha256 = "sha256-yvUbTXKiAcAV0tydsAYi4abHDpE1c3udiq4WSRvLqP8=";
    };
  };
  musicdl = {
    pname = "musicdl";
    version = "0a64818d6395fd468cb08d40c5cd0943bdba5cd0";
    src = fetchFromGitHub {
      owner = "CharlesPikachu";
      repo = "musicdl";
      rev = "0a64818d6395fd468cb08d40c5cd0943bdba5cd0";
      fetchSubmodules = false;
      sha256 = "sha256-aeM259XFPFY3ZBIkUnPiSD3EJVbL8doFmtNCW9FDuPA=";
    };
    date = "20260425";
  };
  musicn = {
    pname = "musicn";
    version = "v1.5.3-beta.0";
    src = fetchFromGitHub {
      owner = "zonemeen";
      repo = "musicn";
      rev = "v1.5.3-beta.0";
      fetchSubmodules = false;
      sha256 = "sha256-loIT6Oy0TmkqtjEUMOVZEk27hr5wWPiP5WycGDXmo3s=";
    };
  };
  nowplaying-cli = {
    pname = "nowplaying-cli";
    version = "v2.1.0";
    src = fetchFromGitHub {
      owner = "kirtan-shah";
      repo = "nowplaying-cli";
      rev = "v2.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-jPW3WEq1ZhxBojMO+5WF8ohO1rLmlAJKGdh1HfSOR5s=";
    };
  };
  nu-lint = {
    pname = "nu-lint";
    version = "a993b41cfdcb8c9ba9498ff867b37a1841509991";
    src = fetchFromGitHub {
      owner = "wvhulle";
      repo = "nu-lint";
      rev = "a993b41cfdcb8c9ba9498ff867b37a1841509991";
      fetchSubmodules = false;
      sha256 = "sha256-vvTpcndPa6+oU7a2DVdJSHwyvhzmxPFxuWwyUeFih5Q=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-vvTpcndPa6+oU7a2DVdJSHwyvhzmxPFxuWwyUeFih5Q=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "20260424";
  };
  pngpaste = {
    pname = "pngpaste";
    version = "0.2.3";
    src = fetchFromGitHub {
      owner = "jcsalterego";
      repo = "pngpaste";
      rev = "0.2.3";
      fetchSubmodules = false;
      sha256 = "sha256-uvajxSelk1Wfd5is5kmT2fzDShlufBgC0PDCeabEOSE=";
    };
  };
  pragmasevka = {
    pname = "pragmasevka";
    version = "34.4.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.4.0/pragma-sevka-mono.zip";
      sha256 = "sha256-KoY578DD+cl6sdSbsIlq6eQVh2tL/OHcYShvk1+0EdQ=";
    };
  };
  pragmasevka-nerd = {
    pname = "pragmasevka-nerd";
    version = "34.4.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.4.0/pragma-sevka-mono-nerd.zip";
      sha256 = "sha256-MKJIHhyZ6PIZYwhI1iQXq5e2H0R/lwJoS5fBxCr7BEg=";
    };
  };
  pragmasevka-sans = {
    pname = "pragmasevka-sans";
    version = "34.4.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.4.0/pragma-sevka-sans.zip";
      sha256 = "sha256-0n0fIqNAFA0DWLzzNKD8wK71vtaVale/c3HxGx+hBS4=";
    };
  };
  pragmasevka-sc = {
    pname = "pragmasevka-sc";
    version = "34.4.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.4.0/Pragma-Sevka-SC-Regular.ttf";
      sha256 = "sha256-CKY+CNMiHkoUV+Aa9E46MIwxEEdk865p/WgfaMiLufA=";
    };
  };
  pragmasevka-sc-nf = {
    pname = "pragmasevka-sc-nf";
    version = "34.4.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.4.0/Pragma-Sevka-SC-NF-Regular.ttf";
      sha256 = "sha256-uC6R2Nuj7sCS5FchzXQ4aDkSHFqXHg+GnD4hxlusecw=";
    };
  };
  pragmasevka-serif = {
    pname = "pragmasevka-serif";
    version = "34.4.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.4.0/pragma-sevka-serif.zip";
      sha256 = "sha256-6kh4fEUZOgay1bOKLamNTdGmxy0AKQnwja2rPVuOYlw=";
    };
  };
  pyfreeproxy = {
    pname = "pyfreeproxy";
    version = "c8f2ac9dbf78b50371e2a4c94e31141357141347";
    src = fetchFromGitHub {
      owner = "CharlesPikachu";
      repo = "freeproxy";
      rev = "c8f2ac9dbf78b50371e2a4c94e31141357141347";
      fetchSubmodules = false;
      sha256 = "sha256-HMQo0phVVp9NmUzydHEcn5GLRUJrc/XUD+KDet7Vx5k=";
    };
    date = "20260424";
  };
  rime-ice = {
    pname = "rime-ice";
    version = "2bd2983c6c74ea49b3a013f150ade7f3b8a27515";
    src = fetchFromGitHub {
      owner = "iDvel";
      repo = "rime-ice";
      rev = "2bd2983c6c74ea49b3a013f150ade7f3b8a27515";
      fetchSubmodules = false;
      sha256 = "sha256-VRMaa5WY7nqIFI6x3lA2PoEFwdUBPGGOl3k4ThSz4RE=";
    };
    date = "20260414";
  };
  sarasa-term = {
    pname = "sarasa-term";
    version = "2.3.1";
    src = fetchurl {
      url = "https://github.com/laishulu/Sarasa-Term-SC-Nerd/releases/download/v2.3.1/SarasaTermSCNerd.ttc.tar.gz";
      sha256 = "sha256-WiXDbXdpRmf4R8IkdeIC3jUuwRKPUFUIHT6CzYoDmuI=";
    };
  };
  sd = {
    pname = "sd";
    version = "748570a6877b028969b16707d32e8dfa58ee49c8";
    src = fetchFromGitHub {
      owner = "shanyouli";
      repo = "sd";
      rev = "748570a6877b028969b16707d32e8dfa58ee49c8";
      fetchSubmodules = false;
      sha256 = "sha256-wl/xsew2WkEMny2u8rSbKPafjwuGg8uRQKnH4ZjnAnY=";
    };
    date = "20260421";
  };
  seam = {
    pname = "seam";
    version = "v_cli.0.1.39";
    src = fetchFromGitHub {
      owner = "Borber";
      repo = "seam";
      rev = "v_cli.0.1.39";
      fetchSubmodules = false;
      sha256 = "sha256-rWbswgVQNA006gi9G+Vma+y0NbMDC0/P6SqHwxxvky8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-rWbswgVQNA006gi9G+Vma+y0NbMDC0_P6SqHwxxvky8=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  sidebery = {
    pname = "sidebery";
    version = "v5.5.2";
    src = fetchFromGitHub {
      owner = "mbnuqw";
      repo = "sidebery";
      rev = "v5.5.2";
      fetchSubmodules = false;
      sha256 = "sha256-V77H6DfLW1Xy3kdjtCSTptfagxo0A6Syv+c3Zm3fJvc=";
    };
  };
  singboxp = {
    pname = "singboxp";
    version = "1bd617e1afff7faa51515cb8a74b63042009f555";
    src = fetchFromGitHub {
      owner = "PuerNya";
      repo = "sing-box";
      rev = "1bd617e1afff7faa51515cb8a74b63042009f555";
      fetchSubmodules = false;
      sha256 = "sha256-KpXyy/eng8dmLvpdMyFnNoBvID0ejIytM4p7WyM08zs=";
    };
    date = "20260112";
  };
  switchaudio-osx = {
    pname = "switchaudio-osx";
    version = "1.2.2";
    src = fetchFromGitHub {
      owner = "deweller";
      repo = "switchaudio-osx";
      rev = "1.2.2";
      fetchSubmodules = false;
      sha256 = "sha256-AZJn5kHK/al94ONfIHcG+W0jyMfgdJkIngN+PVj+I44=";
    };
  };
  tmux-fastcopy = {
    pname = "tmux-fastcopy";
    version = "v0.14.1";
    src = fetchFromGitHub {
      owner = "abhinav";
      repo = "tmux-fastcopy";
      rev = "v0.14.1";
      fetchSubmodules = false;
      sha256 = "sha256-j7Kk5uaC3vx4JifaAHXfLvXx6ask9YqlNKEiU5BYi+A=";
    };
  };
  userChromeJS = {
    pname = "userChromeJS";
    version = "e377756420009e3c04e78642dd00d5e8d7c06690";
    src = fetchFromGitHub {
      owner = "benzBrake";
      repo = "userChrome.js-Loader";
      rev = "e377756420009e3c04e78642dd00d5e8d7c06690";
      fetchSubmodules = false;
      sha256 = "sha256-4NLEl+zx9cZz8htAOTW4WyT6DWNH04qZ1Lj5BVJWDVw=";
    };
    date = "20260407";
  };
  websocket-bridge-python = {
    pname = "websocket-bridge-python";
    version = "0.0.2";
    src = fetchurl {
      url = "https://pypi.org/packages/source/w/websocket_bridge_python/websocket_bridge_python-0.0.2.tar.gz";
      sha256 = "sha256-J9HGD3PSavZQwh2eZPbM6isdknZ1M2nkL5WHlYjRju8=";
    };
  };
  yabai-zsh-completions = {
    pname = "yabai-zsh-completions";
    version = "6e38681a002e13bdcd43f461f73c53b7c11fd4e5";
    src = fetchFromGitHub {
      owner = "Amar1729";
      repo = "yabai-zsh-completions";
      rev = "6e38681a002e13bdcd43f461f73c53b7c11fd4e5";
      fetchSubmodules = false;
      sha256 = "sha256-II00E32Pnt7PO+PcTtWp4NzSUDhQJTgAPw9HdlItbhQ=";
    };
    date = "20231113";
  };
  zpmod = {
    pname = "zpmod";
    version = "v1.1.0";
    src = fetchFromGitHub {
      owner = "z-shell";
      repo = "zpmod";
      rev = "v1.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-vCQOn+8l9DcVJ6XbtHhr7N5IDr2Ht/JRub217ZO96Ok=";
    };
  };
}
