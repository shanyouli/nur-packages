# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  alive-progress = {
    pname = "alive-progress";
    version = "3.3.0";
    src = fetchurl {
      url = "https://pypi.org/packages/source/a/alive-progress/alive-progress-3.3.0.tar.gz";
      sha256 = "sha256-RX3SQotI2s1JhUAipGRI0jakjxtyd4dAccOTlTB+gww=";
    };
  };
  antifennel = {
    pname = "antifennel";
    version = "cea2f62b71c53eeb2bfe056625a87f1a9ddfcddf";
    src = fetchgit {
      url = "https://git.sr.ht/~technomancy/antifennel";
      rev = "cea2f62b71c53eeb2bfe056625a87f1a9ddfcddf";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-LPQLxq7WQHi6CXgdYy6FKz04efFT5p6xPwDx5qhAw50=";
    };
    date = "20251005";
  };
  bash-env-json = {
    pname = "bash-env-json";
    version = "0.7.0";
    src = fetchurl {
      url = "https://github.com/tesujimath/bash-env-json/raw/0.7.0/bash-env-json";
      sha256 = "sha256-6x8zErIYZuk3qWB807rNVbvH9ZLl8cGffJnKrt7DlSg=";
    };
  };
  bbdown = {
    pname = "bbdown";
    version = "259a5558cee0a349a7ebb60bd31e40c88e5bc1ed";
    src = fetchFromGitHub {
      owner = "nilaoda";
      repo = "BBDown";
      rev = "259a5558cee0a349a7ebb60bd31e40c88e5bc1ed";
      fetchSubmodules = false;
      sha256 = "sha256-5c+tvWOx9BMOfqZGGkPw86abhfezYXVpz1J/RLEQnA8=";
    };
    date = "20260110";
  };
  borders = {
    pname = "borders";
    version = "v1.8.4";
    src = fetchFromGitHub {
      owner = "FelixKratz";
      repo = "JankyBorders";
      rev = "v1.8.4";
      fetchSubmodules = false;
      sha256 = "sha256-31Er+cUQNJbZnXKC6KvlrBhOvyPAM7nP3BaxunAtvWg=";
    };
  };
  clash2singbox = {
    pname = "clash2singbox";
    version = "v0.1.4";
    src = fetchFromGitHub {
      owner = "xmdhs";
      repo = "clash2singbox";
      rev = "v0.1.4";
      fetchSubmodules = false;
      sha256 = "sha256-qEv5lmuu4UWaxmCQ0sb21TMJVhYaW7VscD6O+dOWjgE=";
    };
  };
  darkreader = {
    pname = "darkreader";
    version = "4.9.124";
    src = fetchurl {
      url = "https://github.com/darkreader/darkreader/releases/download/v4.9.124/darkreader-firefox.xpi";
      sha256 = "sha256-0RS8qFtPsuOHCnx4Z67wtQc4rNNltYipRMrtrwCpKQ4=";
    };
  };
  deeplx = {
    pname = "deeplx";
    version = "v1.1.1";
    src = fetchFromGitHub {
      owner = "OwO-Network";
      repo = "DeepLX";
      rev = "v1.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-M51z7iXJmYSRc9bsVPsbhD3v8Fq8Gwg+CO37B18wRZo=";
    };
  };
  dict2xml = {
    pname = "dict2xml";
    version = "release-1.7.8";
    src = fetchFromGitHub {
      owner = "delfick";
      repo = "python-dict2xml";
      rev = "release-1.7.8";
      fetchSubmodules = false;
      sha256 = "sha256-wCspFcqn6uAvecxx4Agzg7N3ps82mg8ukmmGwhfgajk=";
    };
  };
  dutis = {
    pname = "dutis";
    version = "v2.3.1";
    src = fetchFromGitHub {
      owner = "tsonglew";
      repo = "dutis";
      rev = "v2.3.1";
      fetchSubmodules = false;
      sha256 = "sha256-90BVsQdFCC+uqvd1EvY/t+242AmQuTalD5dSiA4n2aA=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-90BVsQdFCC+uqvd1EvY_t+242AmQuTalD5dSiA4n2aA=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  emacs-reader = {
    pname = "emacs-reader";
    version = "98c5046683e997902a83092b65cdb70ab120e000";
    src = fetchgit {
      url = "https://codeberg.org/divyaranjan/emacs-reader";
      rev = "98c5046683e997902a83092b65cdb70ab120e000";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Jo8ZecM4Y22T5kc5zJzCvSywkxwcpNEtQ3HHMJNesac=";
    };
    date = "20260217";
  };
  "emacs29.no-frame-refocus-cocoa" = {
    pname = "emacs29.no-frame-refocus-cocoa";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-28/no-frame-refocus-cocoa.patch";
      sha256 = "sha256-+1d33IkKoHNJ8UOuZcK89D7a1v6/1WSwGiI1xaFfyr0=";
    };
    date = "2026-04-05";
  };
  "emacs29.role-patch" = {
    pname = "emacs29.role-patch";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-28/fix-window-role.patch";
      sha256 = "sha256-H4Qj6n5uZsmsbdjjexGZctqhJk3gAXKiSnmnEO/LgTA=";
    };
    date = "2026-04-05";
  };
  "emacs29.round-undecorated-frame" = {
    pname = "emacs29.round-undecorated-frame";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-29/round-undecorated-frame.patch";
      sha256 = "sha256-dFH4D1WYQOVOagUuVdEQB3irxV+Y8dDAOKJOJXc/KHQ=";
    };
    date = "2026-04-05";
  };
  "emacs29.system-appearance" = {
    pname = "emacs29.system-appearance";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-28/system-appearance.patch";
      sha256 = "sha256-1u4VmDmzi2r1Ode5vf8jEmPkUcH9Qu7A0SUxjJ24zZI=";
    };
    date = "2026-04-05";
  };
  "emacs30.ns-mac-input-source" = {
    pname = "emacs30.ns-mac-input-source";
    version = "a5ae72eda20db84558596add9c95978729848a08";
    src = fetchurl {
      url = "https://github.com/LuciusChen/.emacs.d/raw/a5ae72eda20db84558596add9c95978729848a08/scripts/macos/patches/emacs-30/ns-alpha-background.patch";
      sha256 = "sha256-izzfjAWg4e8yYDuODXKIgwG7TdQUg2Su6ZebyzCg9hM=";
    };
    date = "2026-03-30";
  };
  "emacs30.role-patch" = {
    pname = "emacs30.role-patch";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-28/fix-window-role.patch";
      sha256 = "sha256-H4Qj6n5uZsmsbdjjexGZctqhJk3gAXKiSnmnEO/LgTA=";
    };
    date = "2026-04-05";
  };
  "emacs30.round-undecorated-frame" = {
    pname = "emacs30.round-undecorated-frame";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-30/round-undecorated-frame.patch";
      sha256 = "sha256-dFH4D1WYQOVOagUuVdEQB3irxV+Y8dDAOKJOJXc/KHQ=";
    };
    date = "2026-04-05";
  };
  "emacs30.system-appearance" = {
    pname = "emacs30.system-appearance";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-30/system-appearance.patch";
      sha256 = "sha256-nrPOgGQAJb/5brrrWJNDARY2jWNJ9OsMtO+LPVhHfbY=";
    };
    date = "2026-04-05";
  };
  "emacs31.ns-mac-input-source" = {
    pname = "emacs31.ns-mac-input-source";
    version = "a5ae72eda20db84558596add9c95978729848a08";
    src = fetchurl {
      url = "https://github.com/LuciusChen/.emacs.d/raw/a5ae72eda20db84558596add9c95978729848a08/scripts/macos/patches/emacs-31/ns-mac-input-source.patch";
      sha256 = "sha256-mf77qesHJXQ6kBzZXg3rV1GT4dJYCUy1bMusN62g/6I=";
    };
    date = "2026-03-30";
  };
  "emacs31.round-undecorated-frame" = {
    pname = "emacs31.round-undecorated-frame";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-31/round-undecorated-frame.patch";
      sha256 = "sha256-JpR7ZyT8KfrdRIiYCMXPC0zmJ4zwT0YIaiHfUMjEFR0=";
    };
    date = "2026-04-05";
  };
  "emacs31.system-appearance" = {
    pname = "emacs31.system-appearance";
    version = "be36e1ea6537cb312d2aa36483038217b5882bb0";
    src = fetchurl {
      url = "https://github.com/d12frosted/homebrew-emacs-plus/raw/be36e1ea6537cb312d2aa36483038217b5882bb0/patches/emacs-31/system-appearance.patch";
      sha256 = "sha256-Uyg1A9te0oh+nXM7qq+A8sgQ5mjngumIvaWFWgsevrQ=";
    };
    date = "2026-04-05";
  };
  emacsclient = {
    pname = "emacsclient";
    version = "917f2f2694019d534098f5e2e365b5f6e5ddbd37";
    src = fetchurl {
      url = "https://github.com/nashamri/spacemacs-logo/raw/917f2f2694019d534098f5e2e365b5f6e5ddbd37/spacemacs.icns";
      sha256 = "sha256-s9uLfPpLxbziS8TcHt47dSxxhse1TAmZTqtexOqkiQA=";
    };
    date = "2018-08-26";
  };
  emt = {
    pname = "emt";
    version = "b7f46fae280dba3269e9110f3cb61d2dd4fa67cc";
    src = fetchFromGitHub {
      owner = "roife";
      repo = "emt";
      rev = "b7f46fae280dba3269e9110f3cb61d2dd4fa67cc";
      fetchSubmodules = false;
      sha256 = "sha256-ewHZxytYw2NPkhF4zdl/fRSiBSN2jZAkdM4Ykb1x90Q=";
    };
    date = "20250102";
  };
  ewt-rs = {
    pname = "ewt-rs";
    version = "v0.4.0";
    src = fetchFromGitHub {
      owner = "Master-Hash";
      repo = "ewt-rs";
      rev = "v0.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-Fo6MsMoMkTs/dyI5MgoqsWn79ZKP+ZO9XVew06hL4zo=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Fo6MsMoMkTs_dyI5MgoqsWn79ZKP+ZO9XVew06hL4zo=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  fav = {
    pname = "fav";
    version = "v1.2.4";
    src = fetchFromGitHub {
      owner = "kingwingfly";
      repo = "fav";
      rev = "v1.2.4";
      fetchSubmodules = false;
      sha256 = "sha256-BGYSPIDEOlG+CYxfCVikvWLeyMT6rRTnN2xrGXdBsds=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-BGYSPIDEOlG+CYxfCVikvWLeyMT6rRTnN2xrGXdBsds=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  fisher = {
    pname = "fisher";
    version = "4.4.8";
    src = fetchFromGitHub {
      owner = "jorgebucaran";
      repo = "fisher";
      rev = "4.4.8";
      fetchSubmodules = false;
      sha256 = "sha256-Sf671UGOQXtOMrqoEOIBG5TCt0p5fd+aKGF2ExImbbs=";
    };
  };
  flexfox = {
    pname = "flexfox";
    version = "v6.0.6";
    src = fetchFromGitHub {
      owner = "yuuqilin";
      repo = "FlexFox";
      rev = "v6.0.6";
      fetchSubmodules = false;
      sha256 = "sha256-DmrvmQmhYvmAMRGg9aA+lyuwnO3HY8GkM4RBEnWP9ZU=";
    };
  };
  go-musicfox = {
    pname = "go-musicfox";
    version = "v4.8.1";
    src = fetchFromGitHub {
      owner = "go-musicfox";
      repo = "go-musicfox";
      rev = "v4.8.1";
      fetchSubmodules = false;
      sha256 = "sha256-EwN8tWoyghG9L++Tl5iz2ZyNsI5IroZXM0Dd5N182dU=";
    };
  };
  httpx = {
    pname = "httpx";
    version = "0.28.1";
    src = fetchFromGitHub {
      owner = "encode";
      repo = "httpx";
      rev = "0.28.1";
      fetchSubmodules = false;
      sha256 = "sha256-tB8uZm0kPRnmeOvsDdrkrHcMVIYfGanB4l/xHsTKpgE=";
    };
  };
  jeejah = {
    pname = "jeejah";
    version = "c9c45fb4844035ffa71f33d8a571a532a6a82fd7";
    src = fetchgit {
      url = "https://gitlab.com/technomancy/jeejah";
      rev = "c9c45fb4844035ffa71f33d8a571a532a6a82fd7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-qNmLqHfHY1ksMpGFmIHQpzS3QKG81CGSnM79GaqZRFE=";
    };
    date = "20251116";
  };
  jeejah-rock = {
    pname = "jeejah-rock";
    version = "0.3.2-1";
    src = fetchurl {
      url = "https://luarocks.org/manifests/technomancy/jeejah-0.3.2-1.rockspec";
      sha256 = "sha256-Ouj+umHFOLtGwwMKochvG1fqIyRMdPzXg9039jD/bPk=";
    };
  };
  lporg = {
    pname = "lporg";
    version = "v20.4.32";
    src = fetchFromGitHub {
      owner = "blacktop";
      repo = "lporg";
      rev = "v20.4.32";
      fetchSubmodules = false;
      sha256 = "sha256-g8zZZ/uQeoJQIzpQY3oMUuWMu4/8OnaOLeLvaii+Tmk=";
    };
  };
  mac-app-util = {
    pname = "mac-app-util";
    version = "4747968574ea58512c5385466400b2364c85d2d0";
    src = fetchurl {
      url = "https://github.com/hraban/mac-app-util/raw/4747968574ea58512c5385466400b2364c85d2d0/main.lisp";
      sha256 = "sha256-mDFxuZN2y3+FcMWh3YdmC0ZjjRfZxD7K6QbqX8BzkWI=";
    };
    date = "2025-12-26";
  };
  manix = {
    pname = "manix";
    version = "5a69f920f1501f3f7e71a275ab8dbfd4a9f85fb3";
    src = fetchFromGitHub {
      owner = "kaii-zen";
      repo = "manix";
      rev = "5a69f920f1501f3f7e71a275ab8dbfd4a9f85fb3";
      fetchSubmodules = false;
      sha256 = "sha256-Qxs6p+MyS/0NTbJzLKvZHgKBUE1+z+zaINouG//Uio8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Qxs6p+MyS_0NTbJzLKvZHgKBUE1+z+zaINouG__Uio8=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "20210728";
  };
  mkalias = {
    pname = "mkalias";
    version = "v0.4.0";
    src = fetchFromGitHub {
      owner = "reckenrode";
      repo = "mkAlias";
      rev = "v0.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-YUvxBEwXu1LSYC+7g6inBizqgdjKnLugUTXtpSfayxI=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-YUvxBEwXu1LSYC+7g6inBizqgdjKnLugUTXtpSfayxI=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  "mtranserver.linux.amd64" = {
    pname = "mtranserver.linux.amd64";
    version = "4.0.33";
    src = fetchurl {
      url = "https://github.com/xxnuo/MTranServer/releases/download/v4.0.33/mtranserver-4.0.33-linux-amd64";
      sha256 = "sha256-nVbDsdMckZOAUO51D39LqO96oXiHLXGcGYm+SAXPu5U=";
    };
  };
  "mtranserver.linux.arm64" = {
    pname = "mtranserver.linux.arm64";
    version = "4.0.33";
    src = fetchurl {
      url = "https://github.com/xxnuo/MTranServer/releases/download/v4.0.33/mtranserver-4.0.33-linux-arm64";
      sha256 = "sha256-Yrbkhjj1Akvp4MPSB5gU7wU2LBVms47sOnBrggY8Pys=";
    };
  };
  "mtranserver.mac.amd64" = {
    pname = "mtranserver.mac.amd64";
    version = "4.0.33";
    src = fetchurl {
      url = "https://github.com/xxnuo/MTranServer/releases/download/v4.0.33/mtranserver-4.0.33-darwin-amd64";
      sha256 = "sha256-jfwUg3V1I3fpzMZFn6DYGducPoc6ote0eKUk7f9LIDw=";
    };
  };
  "mtranserver.mac.arm64" = {
    pname = "mtranserver.mac.arm64";
    version = "4.0.33";
    src = fetchurl {
      url = "https://github.com/xxnuo/MTranServer/releases/download/v4.0.33/mtranserver-4.0.33-darwin-arm64";
      sha256 = "sha256-yvUbTXKiAcAV0tydsAYi4abHDpE1c3udiq4WSRvLqP8=";
    };
  };
  musicdl = {
    pname = "musicdl";
    version = "18ef6c8354541ee4e05ee0c243173f8b46b46480";
    src = fetchFromGitHub {
      owner = "CharlesPikachu";
      repo = "musicdl";
      rev = "18ef6c8354541ee4e05ee0c243173f8b46b46480";
      fetchSubmodules = false;
      sha256 = "sha256-8YhB/XZVyJ6q9Jj9GkVK2Q9UdxLwFK6WiUAcBCit4L8=";
    };
    date = "20260404";
  };
  musicn = {
    pname = "musicn";
    version = "v1.5.3-beta.0";
    src = fetchFromGitHub {
      owner = "zonemeen";
      repo = "musicn";
      rev = "v1.5.3-beta.0";
      fetchSubmodules = false;
      sha256 = "sha256-loIT6Oy0TmkqtjEUMOVZEk27hr5wWPiP5WycGDXmo3s=";
    };
  };
  nowplaying-cli = {
    pname = "nowplaying-cli";
    version = "v2.0.0";
    src = fetchFromGitHub {
      owner = "kirtan-shah";
      repo = "nowplaying-cli";
      rev = "v2.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-hP5/4Q2NrBOxciHObIL51MHSjNzeuXZqo/cWo4nRQKE=";
    };
  };
  nu-lint = {
    pname = "nu-lint";
    version = "526d92568c634f62d686f6beab0ac50697fe4dc0";
    src = fetchFromGitHub {
      owner = "wvhulle";
      repo = "nu-lint";
      rev = "526d92568c634f62d686f6beab0ac50697fe4dc0";
      fetchSubmodules = false;
      sha256 = "sha256-T3apix4ltkGNiDvnth0GgtXPDN7D8AEnD7fo+uz0+6I=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-T3apix4ltkGNiDvnth0GgtXPDN7D8AEnD7fo+uz0+6I=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "20260404";
  };
  pngpaste = {
    pname = "pngpaste";
    version = "0.2.3";
    src = fetchFromGitHub {
      owner = "jcsalterego";
      repo = "pngpaste";
      rev = "0.2.3";
      fetchSubmodules = false;
      sha256 = "sha256-uvajxSelk1Wfd5is5kmT2fzDShlufBgC0PDCeabEOSE=";
    };
  };
  pragmasevka = {
    pname = "pragmasevka";
    version = "34.3.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.3.0/pragma-sevka-mono.zip";
      sha256 = "sha256-iJnyPFQCVhApTG+mupISe6NUmHicVcdZ5tVaOJt81Rg=";
    };
  };
  pragmasevka-nerd = {
    pname = "pragmasevka-nerd";
    version = "34.3.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.3.0/pragma-sevka-mono-nerd.zip";
      sha256 = "sha256-Nt6hgxblgOGI/UOQIQzknCxTpLn+traHDW2bwAK6uwY=";
    };
  };
  pragmasevka-sans = {
    pname = "pragmasevka-sans";
    version = "34.3.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.3.0/pragma-sevka-sans.zip";
      sha256 = "sha256-off7l+QpttnupfV0A+5XOSLqP6QGBYdTP0K0j5dWtw0=";
    };
  };
  pragmasevka-sc = {
    pname = "pragmasevka-sc";
    version = "34.3.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.3.0/Pragma-Sevka-SC-Regular.ttf";
      sha256 = "sha256-CtFokYzNYUlVUTnlS6OGkKn/CmNWeSYmeEZv3bQr8kw=";
    };
  };
  pragmasevka-sc-nf = {
    pname = "pragmasevka-sc-nf";
    version = "34.3.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.3.0/Pragma-Sevka-SC-NF-Regular.ttf";
      sha256 = "sha256-RPzTHUIlQj+jUPd7ceUOrQyHjDGxSotGbBt87MAE0VY=";
    };
  };
  pragmasevka-serif = {
    pname = "pragmasevka-serif";
    version = "34.3.0";
    src = fetchurl {
      url = "https://github.com/shanyouli/iosevka/releases/download/v34.3.0/pragma-sevka-serif.zip";
      sha256 = "sha256-YXi9i3t0w8qXHtncdt2eIcim+8N2VwJX1nXZ4k9PLxA=";
    };
  };
  proxychains-rs = {
    pname = "proxychains-rs";
    version = "v0.0.10";
    src = fetchFromGitHub {
      owner = "tianrking";
      repo = "proxychains-rs";
      rev = "v0.0.10";
      fetchSubmodules = false;
      sha256 = "sha256-v51vh+ebMowMr9AaCGCyJ/ZmFEtcUvA5qHdcA2vE/8o=";
    };
  };
  pyfreeproxy = {
    pname = "pyfreeproxy";
    version = "5bf6940dba5b2f2b3b53cfcd6a70c8eee587529b";
    src = fetchFromGitHub {
      owner = "CharlesPikachu";
      repo = "freeproxy";
      rev = "5bf6940dba5b2f2b3b53cfcd6a70c8eee587529b";
      fetchSubmodules = false;
      sha256 = "sha256-qtRzSCC++jHlUD+Hqa4LRX/ItQLOZTC/Cmr/4eOQ49I=";
    };
    date = "20260405";
  };
  rime-ice = {
    pname = "rime-ice";
    version = "4c15933d984bc9fffaba898379a2db88483cf7c5";
    src = fetchFromGitHub {
      owner = "iDvel";
      repo = "rime-ice";
      rev = "4c15933d984bc9fffaba898379a2db88483cf7c5";
      fetchSubmodules = false;
      sha256 = "sha256-pl3hCwKRzFEfWW9w8mEAM1OaqhFMFyjsFQiKmB4fbSQ=";
    };
    date = "20260402";
  };
  sarasa-term = {
    pname = "sarasa-term";
    version = "2.3.1";
    src = fetchurl {
      url = "https://github.com/laishulu/Sarasa-Term-SC-Nerd/releases/download/v2.3.1/SarasaTermSCNerd.ttc.tar.gz";
      sha256 = "sha256-WiXDbXdpRmf4R8IkdeIC3jUuwRKPUFUIHT6CzYoDmuI=";
    };
  };
  sd = {
    pname = "sd";
    version = "f63534a5b6057a8dab4f11e1361259acbf02da79";
    src = fetchFromGitHub {
      owner = "shanyouli";
      repo = "sd";
      rev = "f63534a5b6057a8dab4f11e1361259acbf02da79";
      fetchSubmodules = false;
      sha256 = "sha256-rDrL2zaexSkoTWNenI6boStXRVLneoV5KjPFza+KOZ4=";
    };
    date = "20260118";
  };
  seam = {
    pname = "seam";
    version = "v_cli.0.1.39";
    src = fetchFromGitHub {
      owner = "Borber";
      repo = "seam";
      rev = "v_cli.0.1.39";
      fetchSubmodules = false;
      sha256 = "sha256-rWbswgVQNA006gi9G+Vma+y0NbMDC0/P6SqHwxxvky8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-rWbswgVQNA006gi9G+Vma+y0NbMDC0_P6SqHwxxvky8=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  sidebery = {
    pname = "sidebery";
    version = "v5.5.0";
    src = fetchFromGitHub {
      owner = "mbnuqw";
      repo = "sidebery";
      rev = "v5.5.0";
      fetchSubmodules = false;
      sha256 = "sha256-u3LDhDobYBTKZVdR9YpNHHE/YuHPmv1LnEzkAQNjRuY=";
    };
  };
  singboxp = {
    pname = "singboxp";
    version = "1bd617e1afff7faa51515cb8a74b63042009f555";
    src = fetchFromGitHub {
      owner = "PuerNya";
      repo = "sing-box";
      rev = "1bd617e1afff7faa51515cb8a74b63042009f555";
      fetchSubmodules = false;
      sha256 = "sha256-KpXyy/eng8dmLvpdMyFnNoBvID0ejIytM4p7WyM08zs=";
    };
    date = "20260112";
  };
  switchaudio-osx = {
    pname = "switchaudio-osx";
    version = "1.2.2";
    src = fetchFromGitHub {
      owner = "deweller";
      repo = "switchaudio-osx";
      rev = "1.2.2";
      fetchSubmodules = false;
      sha256 = "sha256-AZJn5kHK/al94ONfIHcG+W0jyMfgdJkIngN+PVj+I44=";
    };
  };
  tmux-fastcopy = {
    pname = "tmux-fastcopy";
    version = "v0.14.1";
    src = fetchFromGitHub {
      owner = "abhinav";
      repo = "tmux-fastcopy";
      rev = "v0.14.1";
      fetchSubmodules = false;
      sha256 = "sha256-j7Kk5uaC3vx4JifaAHXfLvXx6ask9YqlNKEiU5BYi+A=";
    };
  };
  userChromeJS = {
    pname = "userChromeJS";
    version = "e19ddce9357d4aff1d8ec6aefbff4ab10c283154";
    src = fetchFromGitHub {
      owner = "benzBrake";
      repo = "userChrome.js-Loader";
      rev = "e19ddce9357d4aff1d8ec6aefbff4ab10c283154";
      fetchSubmodules = false;
      sha256 = "sha256-hIFva2nP9c2e36oi91N4Z1VsSwPtb6NUn3dhnz9wCdY=";
    };
    date = "20260328";
  };
  websocket-bridge-python = {
    pname = "websocket-bridge-python";
    version = "0.0.2";
    src = fetchurl {
      url = "https://pypi.org/packages/source/w/websocket_bridge_python/websocket_bridge_python-0.0.2.tar.gz";
      sha256 = "sha256-J9HGD3PSavZQwh2eZPbM6isdknZ1M2nkL5WHlYjRju8=";
    };
  };
  yabai-zsh-completions = {
    pname = "yabai-zsh-completions";
    version = "6e38681a002e13bdcd43f461f73c53b7c11fd4e5";
    src = fetchFromGitHub {
      owner = "Amar1729";
      repo = "yabai-zsh-completions";
      rev = "6e38681a002e13bdcd43f461f73c53b7c11fd4e5";
      fetchSubmodules = false;
      sha256 = "sha256-II00E32Pnt7PO+PcTtWp4NzSUDhQJTgAPw9HdlItbhQ=";
    };
    date = "20231113";
  };
  zpmod = {
    pname = "zpmod";
    version = "v1.1.0";
    src = fetchFromGitHub {
      owner = "z-shell";
      repo = "zpmod";
      rev = "v1.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-vCQOn+8l9DcVJ6XbtHhr7N5IDr2Ht/JRub217ZO96Ok=";
    };
  };
}
