...
let
  ...
  isNixFile =
    n: v: v == "regular" && n != "default.nix" && (hasSuffix ".nix" n) && !(hasPrefix "_" n);
in
rec {
  ...
  fileFn =
    ...
    let
      fpath = toString f;
    in
    if pathIsDirectory fpath then
      fn (fpath + "/default.nix")
    else if pathExists (fpath + ".nix") then
      fn (fpath + ".nix")
    else
      throw "unknown path ${f}";
  ...
  moduleFilesRec' =
    ...
    let
      dirs =
        ...
        in
        mapAttrsToList (k: _: "${p}/${k}") (
          filterAttrs (n: v: v == "directory" && !(hasPrefix "_" n)) (readDir p)
        );
      files = attrValues (moduleFiles dir);
    in
    files ++ concatLists (map (d: (moduleFilesRec' d)) dirs);
