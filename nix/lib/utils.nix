{
  ...
  inherit (builtins) isAttrs attrValues map attrNames filter concatStringsSep;
  inherit (lib) concatMap nameValuePair listToAttrs splitString length removePrefix sublist;
    ...
    lib.mapAttrs (_: v:
      ...
      then v
        ...
        unstableDateVersion =
          ...
          else {rawVersion = v.version;};
      in
        lib.foldl lib.recursiveUpdate v [removeVersionPrefix unstableDateVersion])
    sources;
}
